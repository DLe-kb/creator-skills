...
读取 [platform-routes.md](references/platform-routes.md) 了解费用和能力边界。读取 [platform-fields.md](references/platform-fields.md) 核对平台字段与规格。需要复核权威资料时读取 [source-notes.md](references/source-notes.md)。读取 [security-and-status.md](references/security-and-status.md) 处理凭证、重复发布和结果状态。
